...
// Cache CodeMirror instance to prevent reinitialization
let cachedEditor = null;

// ---- Extracted scripts from inline <script> blocks ----
// Initialize layout checker page when called by SPA router
function initLayoutCheckerPage() {
...
if (!window.spaRouter) {
  document.addEventListener('DOMContentLoaded', initLayoutCheckerPage);
}

// Initialize CodeMirror editor for htmlInput textarea
const textarea = document.getElementById('htmlInput');
const editor = cachedEditor || CodeMirror.fromTextArea(textarea, {
  mode: 'htmlmixed',
  theme: 'material',
  lineNumbers: true,
  lineWrapping: true,
...
const krhredUnitsContainer = document.getElementById('krhredUnitsContainer');
  ...
  function generateKrhredColumns(htmlContent, showNotification = true) {
    ...
    // Show combined notification for HTML fetch and KRHRED units found
    if (showNotification) {
      if (matches.length > 0) {
  ...
  // Add debounced input listeners to KRHRED input fields
  function setupKrhredInputListeners() {
    const inputs = krhredUnitsContainer.querySelectorAll('input[id^="krhred_unit_"]');
    inputs.forEach(input => {
      // Debounced input handler for validation
